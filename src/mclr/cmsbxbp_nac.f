...
***** Auxiliaries
      INTEGER I,K,L,M,N,IKL,IIM,IJN,IKOL,IIK,IJK,IIL,IJL
      Real*8 TempD, dE_JI

      CALL FZero(bX,(nRoots-1)*nRoots/2)
      I=NACstates(1)
      J=NACstates(2)
      dE_JI = E_Final(J)-E_Final(I)
...
*******Diagonal elements of R_JK * H_KL * R_IK
       bX(IKL)=R(IIK)*R(IJK)*LOK(ILOK)-R(IIL)*R(IJL)*LOK(IKOL)
******* Additional NAC term (Requires only one line)
******* R_JK * <K|L> * R_IL = 
******* R_JL * R_IK - R_JK * R_IL
       bX(IKL)=bX(IKL)+ dE_JI * ( R(IJL)*R(IIK)-R(IJK)*R(IIL) )
*******Off-Diagonal elements of R_JK * H_KL * R_IK
       ...
       Do N=1,M-1
        ...
        bX(IKL)=bX(IKL)+TempD*R(IIM)*R(IJN)
       End Do
       End Do
       bX(IKL)=bX(IKL)*2.0d0
      END DO
...
***** Kind quantities that help
      ...
      DO K=1,nRoots
       ...
       Do L=1, nRoots
        ...
        CALL dAXpY_(nConf1,tempd,W(ipci)%Vec(iLoc2),1,bP(iLoc1),1)
       End Do
       CALL DScal_(nConf1,2.0d0*R((I-1)*nRoots+K)*R((J-1)*nRoots+K),
     & bP(iLoc1),1)
      END DO
      RETURN
      End Subroutine
